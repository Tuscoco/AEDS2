...
public class Main {
    ...
    public static void ordenar(int esq, int dir){
        ...
        int i = esq, j = dir, pivo = array[(esq + dir)/2].getGeneration();
        String pivoNome = array[(esq + dir)/2].getName();

        while(i <= j){

            while((array[i].getGeneration() < pivo) || (array[i].getGeneration() == pivo && array[i].getName().compareTo(pivoNome) < 0)){
                i++;
            }

            while((array[j].getGeneration() > pivo) || (array[j].getGeneration() == pivo && array[j].getName().compareTo(pivoNome) > 0)){
                j--;
            }
